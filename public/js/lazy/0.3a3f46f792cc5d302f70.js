    ...
    methods: {
        ...
        setData: function setData(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                var month = item_time.getMonth().length > 1 ? item_time.getMonth() : "0" + item_time.getMonth();
                var day = item_time.getDate().length > 1 ? item_time.getDate() : "0" + item_time.getDate();
                var one_item = year + "-" + month;
                var one_time = month + "-" + day;
                if (this.testData.indexOf(one_item) != -1) {
                    for (var j = 0; j < this.testData[one_item].item.length; j++) {
                        if (this.testData[one_item].item[j].itemData == item_time) {}
                    }
                } else {
                    this.testData[one_item] = new Object();
                    this.testData[one_item].item = new Array();
                    var item_data = new Object();
                    item_data.itemData = item_time;
                    item_data.partImg = data[i].images;
                    item_data.text = data[i].info;
                    this.testData[one_item].item.push(item_data);
                }
