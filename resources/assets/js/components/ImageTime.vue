  ...
  <div class="content">
    ...
    <timeline>
      <div v-for="(items,index) in testData" :key="index">
        ...
        <timeline-item
          bg-color="#9dd8e0"
          v-for="(part,index1) in items.items" :key="index1"
          v-viewer="{title:false,movable:false,url:getUrl}"
        >
          <div class="image-time-data">
            <p>{{part.itemData}}</p>
          </div>
          <div class="image-time-box" v-for="(url,index2) in part.partImg" :key="index2">
